...
test.describe('site 404 (not-found.tsx)', () => {
  ...
  test('renders fallback UI for unmatched URL (catch-all case)', async ({ page }) => {
    const res = await page.goto('/this/route/totally/does/not/exist');
  ...
  test('redirects to configured path when site.notFoundPath is set', async ({ page, request: _request }) => {
  ...
  test('admin /admin/settings shows the new input', async ({ page, request: _request }) => {
